...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .vp-pin-dot { width: 18px; height: 18px; border-radius: 50%;
  ...
  .vp-numpad-key { background: var(--surface2); border: 1.5px solid var(--border);
    border-radius: 12px; padding: 1rem; font-size: 1.5rem; font-weight: 700;
    cursor: pointer; text-align: center; transition: all .1s; min-height: 60px;
  ...
  .vp-numpad-key.backspace { color: var(--text-muted); font-size: 1.2rem; }
  /* ── Shake ── */
  ...
  .vp-row-label { font-family:'DM Mono',monospace; font-size:.68rem; color:var(--text-muted); }
  /* ── Companion ── */
  ...
  .vp-error { background:var(--red-dim); border:1px solid var(--red-border); border-radius:8px;
    padding:.55rem .85rem; font-family:'DM Mono',monospace; font-size:.72rem;
    color:var(--red); text-align:center; margin-top:.75rem; }
  .vp-hint { font-family:'DM Mono',monospace; font-size:.62rem; color:var(--text-dim);
    text-align:center; line-height:1.8; margin-top:.85rem; }
  /* ── Toast ── */
  .vp-toast { background:var(--green-dim); border:1px solid var(--green-border);
    ...
    font-family:'DM Mono',monospace; font-size:.72rem; color:var(--green); }
  /* ── Header fijo del portal ── */
  ...
  .vp-info { background:rgba(34,211,238,.05); border:1px solid var(--cyan-border);
    border-radius:8px; padding:.6rem .85rem; margin-bottom:.75rem;
    font-family:'DM Mono',monospace; font-size:.7rem; color:var(--text-muted); line-height:1.7; }
`;
...
function LoginScreen({ onLogin }) {
  const [paso, setPaso]         = useState(1); // 1=teléfono, 2=PIN
  ...
  const irAlPin = (e) => {
    e?.preventDefault();
    if (!telValido) { setError("Introduce tu número de teléfono (9 dígitos)"); return; }
    setError("");
    setPaso(2);
  };
  ...
  return (
    <div className="vp-page">
      ...
      <div style={{ textAlign:"center", padding:"2rem 1rem 1.5rem" }}>
        <div style={{ fontSize:"2.8rem", marginBottom:".5rem" }}>🏔️</div>
        <div style={{ fontWeight:800, fontSize:"1.25rem", color:"var(--cyan)", marginBottom:".2rem" }}>
          Trail El Guerrero 2026
        </div>
        <div className="vp-mono" style={{ fontSize:".7rem", color:"var(--text-muted)" }}>
        ...
        {paso === 1 && (
          <div>
            <div className="vp-card">
              <div className="vp-label">Tu número de teléfono</div>
              <form onSubmit={irAlPin}>
                <input
                  ...
                  autoComplete="tel"
                  autoFocus
                  style={{ marginBottom:".75rem", fontSize:"1.3rem", letterSpacing:".05em" }}
                />
                {error && <div className="vp-error">⚠ {error}</div>}
                <button type="submit" className="vp-btn vp-btn-primary"
                  style={{ marginTop:".85rem" }} disabled={!telValido}>
                  Continuar →
                </button>
              </form>
            </div>
          ...
          <div>
            ...
            <div className="vp-card">
              <div style={{ textAlign:"center", marginBottom:"1.25rem" }}>
                <div className="vp-label" style={{ marginBottom:".3rem" }}>Introduce tu PIN</div>
              ...
              {loading && (
                <div style={{ textAlign:"center", marginTop:"1rem",
                  fontFamily:"'DM Mono',monospace", fontSize:".72rem", color:"var(--cyan)" }}>
                  Verificando…
                </div>
              )}
            </div>
            ...
            <div className="vp-hint">
              Tu PIN inicial son los <strong style={{color:"var(--text)"}}>últimos 4 dígitos</strong> de tu teléfono
              {telefono.replace(/\D/g,"").length >= 4 && (
                <> · <span style={{color:"var(--cyan)",fontWeight:700}}>
                  {telefono.replace(/\D/g,"").slice(-4)}
                </span></>
              )}
              <br/>¿Problemas? Contacta con el organizador
            </div>
          </div>
        )}
    ...
    <>
      ...
      <div className="vp-wrap">
        ...
        <div className="vp-card" style={{ borderLeft:`3px solid ${puesto ? "var(--cyan)" : "var(--border)"}` }}>
          ...
          {puesto ? (
            <>
              <div style={{ fontWeight:700, fontSize:"1.05rem", marginBottom:".3rem" }}>{puesto.nombre}</div>
              ...
              {puesto.notas && (
                <div className="vp-mono" style={{ fontSize:".68rem", color:"var(--text-dim)",
            ...
            <div className="vp-mono" style={{ fontSize:".78rem", color:"var(--text-dim)" }}>
              ⏳ Pendiente de asignación. Te informaremos pronto.
            </div>
          )}
        </div>

        {/* ── SECCIÓN 2: Botón YA ESTOY EN MI PUESTO ── */}
        ...
        {/* ── SECCIÓN 3: Compañeros de puesto ── */}
            ...
            {companerosEnPuesto.map((c, i) => {
              ...
              return (
                <div key={i} className="vp-companion">
                  <div className="vp-avatar">{iniciales}</div>
                  <div style={{ flex:1 }}>
                    ...
                    {c.telefono && (
                      <a href={`tel:${c.telefono.replace(/\s/g,"")}`}
                        style={{ fontFamily:"'DM Mono',monospace", fontSize:".72rem",
        ...
        {/* ── SECCIÓN 6: Contacto organizador ── */}
        {(config.telefonoContacto || config.organizador) && (
          <div className="vp-card" style={{marginBottom:".75rem"}}>
            ...
            {config.organizador && (
              <div style={{fontWeight:600, fontSize:".88rem", marginBottom:".2rem"}}>{config.organizador}</div>
            )}
            {config.telefonoContacto && (
              <a href={`tel:${config.telefonoContacto.replace(/\s/g,"")}`}
                style={{fontFamily:"'DM Mono',monospace", fontSize:".9rem",
            ...
            {config.emailContacto && (
              <a href={`mailto:${config.emailContacto}`}
                style={{fontFamily:"'DM Mono',monospace", fontSize:".72rem",
                  color:"var(--text-muted)", textDecoration:"none", display:"block", marginTop:".2rem"}}>
                ✉ {config.emailContacto}
              </a>
            )}
          </div>
        ...
        <button className="vp-btn vp-btn-ghost"
          style={{fontSize:".75rem", minHeight:40, color:"var(--text-dim)", marginBottom:".5rem"}}
...
function CambiarPin({ token, onDone, onCancel }) {
  const [step,   setStep]   = useState(1); // 1=nuevo pin, 2=confirmar
  ...
  return (
    <div className="vp-card" style={{marginBottom:".75rem"}}>
      <div className="vp-card-header">
        <div className="vp-mono" style={{fontWeight:700, fontSize:".82rem"}}>
