...
export const TabPresupuesto = ({
  ...
  removeConcepto,
  reorderConceptos,
  // ECO-08: props del bloque Camisetas — Gastos
  ...
  // Totales COMBINADOS (fijos + variables) por categoría para el resumen
  const totalesCombPorCateg = useMemo(() => {
    const m = {};
    // Fijos activos
    conceptos.filter(c => c.tipo==="fijo" && c.activo).forEach(c => {
      const cat = c.categoria?.trim() || "Sin categoría";
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        {vistaResumen ? (<>
          {/* ── Totales combinados por categoría ────────────────────────── */}
          {Object.keys(totalesCombPorCateg).length > 0 && (
            <div style={{ marginBottom:".85rem" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:0,
                ...
                <div style={{ display:"flex", justifyContent:"space-between",
                  padding:".5rem .65rem", background:"var(--surface3)",
                  ...
                  <span style={{ fontFamily:"var(--font-mono)", fontSize:"var(--fs-sm)", fontWeight:800,
                    color:"var(--cyan)" }}>
                    {fmtN((costesFijos.total??0) + (costesVariables.total??0))} €
                  </span>
                </div>
              </div>
            </div>
  ...
  <Tooltip position="top" text={"Importe total del gasto fijo.\nSe prorratea entre distancias activas en proporción al número de inscritos de cada una."}>
    <span>Coste Total (€)</span><TooltipIcon />
      ...
      {/* ECO-08: bloque Camisetas — Gastos. 6 categorías con toggle propio,
          sustituye al antiguo concepto fijo manual "Camisetas voluntarios". */}
